...
import 'package:your_app_name/src/data/model/model_favorite.dart';
import 'package:your_app_name/src/data/model/model_favorites_detail_list.dart';
...
class UserRepository {
  ...
  static Future<ResultApiModel> deleteProfileImage() async {
    final prefs = await Preferences.openBox();
    int userId = prefs.getKeyValue(Preferences.userId, 0);

    final response = await Api.deleteProfileImage(userId);
    if (response.success) {
  ...
  Future<NotificaitonPreferencesModel?> getNotificationPreferences() async {
    try {
      final result = await Api.getNotificationPreferences(
          await UserDataUtil.getLoggedUserId());
      if (result.success) {
        return NotificaitonPreferencesModel.fromJson(result.data);
      }
    } catch (e, stackTrace) {
      await Sentry.captureException(e, stackTrace: stackTrace);
    }
  ...
  static Future<bool?> updateNotificationPreferences(
    ...
    final notificationPreferenceUpdateRequest =
        ModelNotificationPreferenceUpdateRequest(
            type: type, id: id, enabled: enabled);
    try {
      final result = (id == null && type == null)
          ? await Api.requestPostNotificationPreference(
              await UserDataUtil.getLoggedUserId(),
              notificationPreferenceUpdateRequest)
          : await Api.requestPatchNotificationPreference(
              await UserDataUtil.getLoggedUserId(),
              notificationPreferenceUpdateRequest);
      if (result.success) {
        return true;
  ...
  static Future<void> updateNotificationPreferenceForNewUser(data) async {
    final prefs = await Preferences.openBox();
    final receiveNotification =
        await prefs.getKeyValue(Preferences.receiveNotification, '0');
    if (receiveNotification == "false") {
      await updateNotificationPreferences(null, null, false);
    }
  }
}
