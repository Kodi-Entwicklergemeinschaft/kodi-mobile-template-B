...
import 'package:your_app_name/src/presentation/main/account/dashboard/all_listings/cubit/all_listings_state.dart';
...
class _AllListingsLoadedState extends State<AllListingsLoaded> {
  ...
  @override
  void didUpdateWidget(covariant AllListingsLoaded oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.posts != oldWidget.posts ||
        widget.currentCityFilter != oldWidget.currentCityFilter ||
        widget.currentListingFilter != oldWidget.currentListingFilter) {
      setState(() {
        posts = widget.posts;
        currentCityFilter = widget.currentCityFilter;
        currentListingFilter = widget.currentListingFilter;
                                          ...
                                          Expanded(
                                            child: Column(
                                              ...
                                              children: <Widget>[
                                                ...
                                                if (item.sourceId == 3 || item.sourceId == 4)
                                                  Text(
  ...
  void _openListingStatusActionPopUp(ProductModel item) {
    selectedListingStatusValue =
        Translate.of(context).translate(getStatusTanslation(item.statusId!, null));

    List<DropdownMenuItem<String>> getDropdownItems() {
      final approved = DropdownMenuItem<String>(
        value: Translate.of(context).translate('approved'),
        child: Text(Translate.of(context).translate('approved')),
      );
      final pending = DropdownMenuItem<String>(
        value: Translate.of(context).translate('pending'),
        child: Text(Translate.of(context).translate('pending')),
      );
      final feedback = DropdownMenuItem<String>(
        value: Translate.of(context).translate('feedback'),
        child: Text(Translate.of(context).translate('feedback')),
      );
      ...
      switch (item.statusId) {
        case 1: // approved
          return [approved];
        case 2: // pending
          return [pending, feedback, approved];
        case 3: // feedback
    ...
    final dropdownItems = getDropdownItems();
                  ...
                  SimpleDialogOption(
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        isExpanded: false,
                        hint:
                            Text(Translate.of(context).translate('change_status')),
                        ...
                        onChanged: dropdownItems.length <= 1
                            ...
                            : (chosen) async {
                                if (chosen == null) return;
                                int choice =
                                    int.parse(getStatusTanslation(null, chosen));
                                await context
                                    .read<AddListingCubit>()
                                    .changeStatus(item, choice);
                                selectedListingStatusValue =
                                    Translate.of(context).translate(chosen);
                                Navigator.of(context).pop();
                                _onRefresh();
